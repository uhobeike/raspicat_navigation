...
void WaypointRviz::WaypointRvizVisualization(vector<vector<string>>& waypoint_csv_,
                                             ...
                                             float& waypoint_area_threshold_)
{
  ...
  {
    vec_cnt_index = 0;
    WaypointRviz::WaypointMarkerArraySet(
        ...
        case 0:
          ...
          if (waypoint_csv_[distance(waypoint_csv_.begin(), it_t)].size() == 4)
            waypoint_area.markers[distance(waypoint_csv_.begin(), it_t)].pose.position.x =
                stod(*it);
          if (waypoint_csv_[distance(waypoint_csv_.begin(), it_t)].size() > 4)
            if (waypoint_csv_[distance(waypoint_csv_.begin(), it_t)][4] == "SlopeObstacleAvoidance")
              waypoint_area.markers[distance(waypoint_csv_.begin(), it_t)].pose.position.x =
                  stod(*it);
...
void WaypointRviz::WaypointMarkerArraySet(visualization_msgs::MarkerArray& waypoint_area,
                                          visualization_msgs::MarkerArray& waypoint_number_txt,
